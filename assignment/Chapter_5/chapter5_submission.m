...
for scaleIdx = 1 : numel(scales)
    ...
    bitrate_foremanstill(scaleIdx) = bpp ./ videoLength;
    psnr_foremanstill(scaleIdx) = psnr_temp ./ videoLength;
    fprintf('QP: %.1f bit-rate: %.2f bits/pixel PSNR: %.2fdB\n', ...
        qScale, bitrate_foremanstill(scaleIdx), psnr_foremanstill(scaleIdx))
end


%% E5.2 Encode the frames using video codec 
...
ylabel('PSNR [dB]');
legend([foreman, foreman_still], {'Foreman','Foreman-Still'});
title('R-D Plot');
